...
class HiveServerMetricCollector(MetricCollector):
    ...
    def __init__(self, cluster, url):
        MetricCollector.__init__(self, cluster, url, "hive", "hiveserver2")
    ...
    def setup_node_labels(self, bean, service):
        label = ["cluster", "host", "client_id", "node_id"]
        for metric in self._metrics[service]:
            if metric in bean:
                name = re.sub('[^a-z0-9A-Z]', '_', metric).lower()
                self._hadoop_hiveserver2_metrics[service][metric] = GaugeMetricFamily(
                    "_".join([self._prefix, 'producer_node', name]),
                    self._metrics[service][metric],
                    labels=label)

    def setup_topic_labels(self, bean, service):
        label = ["cluster", "host", "client_id", "topic"]
        for metric in self._metrics[service]:
            if metric in bean:
                name = re.sub('[^a-z0-9A-Z]', '_', metric).lower()
                self._hadoop_hiveserver2_metrics[service][metric] = GaugeMetricFamily(
                    "_".join([self._prefix, 'producer_topic', name]),
                    self._metrics[service][metric],
                    labels=label)

    def setup_producer_labels(self, bean, service):
    ...
    def setup_other_labels(self, bean, service):
        label = ["cluster", "host"]
        for metric in self._metrics[service]:
            if metric in bean:
                name = re.sub('[^a-z0-9A-Z]', '_', metric).lower()
    ...
    def setup_labels(self, beans):
        ...
        for service in self._metrics:
            for i in range(len(beans)):
                ...
                elif 'producer-metrics' == service and 'type=producer-metrics' in beans[i][
                    'name'] or 'kafka-metrics-count' == service and 'type=kafka-metrics-count' in beans[i]['name']:
    ...
    def get_node_metrics(self, bean, service, host):
        client_id = bean['name'].split('client-id=')[1].split(',')[0]
        node_id = bean['name'].split('node-id=')[1].split(',')[0]
        for metric in bean:
            if metric in self._metrics[service]:
                self._hadoop_hiveserver2_metrics[service][metric].add_metric([self._cluster, host, client_id, node_id],
                                                                             bean[metric])
            else:
                continue

    def get_topic_metrics(self, bean, service, host):
        client_id = bean['name'].split('client-id=')[1].split(',')[0]
        topic = bean['name'].split('topic=')[1].split(',')[0]
        for metric in bean:
            if metric in self._metrics[service]:
                self._hadoop_hiveserver2_metrics[service][metric].add_metric([self._cluster, host, client_id, topic],
                                                                             bean[metric])
            else:
                continue

    def get_producer_metrics(self, bean, service, host):
        client_id = bean['name'].split('client-id=')[1].split(',')[0]
        for metric in bean:
            if metric in self._metrics[service]:
                self._hadoop_hiveserver2_metrics[service][metric].add_metric([self._cluster, host, client_id],
                                                                             bean[metric])
            else:
                continue

    def get_other_metrics(self, bean, service, host):
        for metric in bean:
            if metric in self._metrics[service]:
